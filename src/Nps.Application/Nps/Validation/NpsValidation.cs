    ...
    public class NpsClientOpenInputValidation : AbstractValidator<NpsClientOpenInput>
    {
        public NpsClientOpenInputValidation()
        {
            ...
            RuleFor(x => x.DeviceUniqueId)
                ...
                .MaximumLength(50).WithMessage("{PropertyName}长度不能大于{MaxLength}位");

            RuleFor(x => x.OpenPorts).NotNull().NotEmpty().WithMessage("设备需要开通的端口号不能为空");

            RuleFor(x => x.Remark)
                .MaximumLength(100).WithName("设备备注信息")
                .WithMessage("{PropertyName}长度不能大于{MaxLength}个字符");
        }
    }
